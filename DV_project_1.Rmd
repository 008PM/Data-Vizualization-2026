...
Data were filtered to the relevant years or locations to align with questions about temporal trends, seasonality, and geographic burden.​

## 4.2.Data analysis and visualization

Data wrangling was performed using dplyr or tidyverse verbs (e.g., `filter`, `group_by`, `summarise`, `mutate`, `join`) and date handling with lubridate where needed.​

## 4.3.Visual outputs (libraries , charts , purpose)
...
**Resurgence annotated trend (1960–2000)** : Libraries: ggplot2, dplyr, lubridate, scales. Chart: annotated time-series line or area chart. Purpose: highlight vaccine introduction, elimination goals, and the 1989–1991 resurgence period.​
...
**Trajectory of measles (deaths vs cases)** : Libraries: ggplot2, dplyr, scales, viridis. Chart: log–log phase-space scatter or trajectory with year color gradient. Purpose: show how deaths and cases jointly evolved over time.​
...
# 3. Building season_summary with mean, SD, SE, CI 
...
# 4 bar chart of seasonal averages with 95% CI 
...
# Extracting month number (1-12) from the date and add it as a new column
measles_df$month <- month(measles_df$from_date)

# Calculating total cases for each month across all years
...
# Creating an enhanced version with color gradient based on case counts
...
# 5. Keeping the 10 most anomalous years (highest z-scores)
